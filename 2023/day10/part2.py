...
def calc_area(arr):
    ...
    a = 0.5 * np.abs(np.dot(x_vals, np.roll(y_vals, 1)) - np.dot(y_vals, np.roll(x_vals, 1)))
    return a - len(arr) // 2 + 1
...
def main():
    ...
    loop_pos_list = []
    curr_pos = [0,0]
    loop_pos_list.append([start_point[0], start_point[1]])
